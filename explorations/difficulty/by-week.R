...
for (idx in 1:nrow(result)) {
    ...
    result$num_blocks[idx] <- diff(range(data$blocknumber[data$timestamp >= as.numeric(date-days(n_days)) & data$timestamp <= as.numeric(date)]))
...
per_31_ts <- c(1485904756,1533578575,1562446275,1616567660,1616567660,1639440039)
per_31s <- as_date(as_datetime(per_31_ts))

time_blocks <- c((n_days*24*60*60) / 14,
                 (n_days*24*60*60) / 16,
                 (n_days*24*60*60) / 18,
                 (n_days*24*60*60) / 20,
                 (n_days*24*60*60) / 22,
